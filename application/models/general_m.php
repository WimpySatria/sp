...
class General_m extends CI_Model {
	...
	//panggil data jenis tabungan dengan id
	...
	//panggil data jenis tabungan
	function get_id_tabungan() {
		$this->db->select('*');
		$this->db->from('jns_tabungan');
		$this->db->where('tampil', 'Y');
		$this->db->order_by('id', 'ASC');
	...
	//menghitung jumlah pokok dan bunga
	// function get_jml_bayar($id) {
	// 	$this->db->select('SUM(bayar_pokok + bayar_bunga) AS total');
	// 	$this->db->from('tbl_pinjaman_d');
	// 	$this->db->where('pinjam_id',$id);
	// 	$query = $this->db->get();
	// 	return $query->row();
