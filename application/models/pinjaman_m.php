...
class Pinjaman_m extends CI_Model {
	...
	// function get_tot_pokok($pinjam_id) {
	// 	$this->db->select('SUM(bayar_pokok) AS total');
	// 	$this->db->from('tbl_pinjaman_d');
	// 	$this->db->where('pinjam_id', $pinjam_id);
	// 	$query = $this->db->get();
	// 	$row = $query->row();
	...
	// function get_jml_bayar($pinjam_id) {
	// 	$this->db->select('SUM(bayar_bunga) AS total');
	// 	$this->db->from('tbl_pinjaman_d');
	// 	$this->db->where('pinjam_id', $pinjam_id);
	// 	$query = $this->db->get();
	// 	$row = $query->row();
	...
	function pengajuan_aksi() {
		...
		// inisialisasi variabel data input
		...
		switch ($status) {
			...
			case 'Setuju':
				...
				$setujui = $this->db->get_where('tbl_pengajuan',['id' => $id])->row();
				
				//siapkan data inputan ke tabel pinjaman_h
				$data_pinjaman_h = array(
					'id' => '',
					'tgl_pinjam' => $tanggal_u,
					'anggota_id' => $setujui->anggota_id,
					'barang_id' => '',
					'lama_angsuran' => $setujui->lama_ags,
					'jumlah' => $setujui->nominal,
					'bunga' => $bunga,
					'biaya_adm' => 0,
					'lunas' => 'Belum',
					'dk' => 'K',
					...
					'user_name' => $setujui->surveyor,
					'keterangan' => $alasan,
					'contoh' => ''

				);

				$this->db->insert('tbl_pinjaman_h', $data_pinjaman_h);
	...
	function get_data_pengajuan($id) {
		...
		$this->db->join('tbl_anggota', 'tbl_anggota.id = tbl_pengajuan.anggota_id');
		$this->db->where('tbl_pengajuan.id', $id);
		$query = $this->db->get();
		...
		if($row) {
			...
			for ($i=1; $i <= $row->lama_angsuran; $i++) { 
				...
				*/
				...
				$odat['bunga_pinjaman'] = $row->bunga_pinjaman;
				$odat['jumlah_ags'] = $row->ags_per_bulan;
				$tgl_tempo_var = substr($row->tgl_pinjam, 0, 7);
