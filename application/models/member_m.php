		...
		if(is_array($data)) {
			...
			foreach ($data as $rows) {
				...
				foreach ($rows as $key => $val) {
					...
					if($key == 'I') { $pair['pekerjaan'] = $val; }
					if($key == 'J') { $pair['alamat'] = $val; }
					if($key == 'K') { $pair['kota'] = $val; }
					if($key == 'L') { $pair['notelp'] = $val; }
					if($key == 'M') { $pair['tgl_daftar'] = $val; }
					if($key == 'N') { $pair['pass_word'] = sha1('nsi' . $val); }
					if($key == 'O') { $pair['departement'] = $val; }
				}
				$pair['jabatan_id'] = 2;
				$pair_arr[] = $pair;
			}
