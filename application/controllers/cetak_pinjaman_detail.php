	...
	function cetak($id) {
		...
		if(!empty($simulasi_tagihan)) {
			...
			foreach ($simulasi_tagihan as $row) {
				...
				$html .= '
					<tr>
						...
						<td class="h_tengah">'.number_format(nsi_round($row['angsuran_pokok'])).'</td>
						<td class="h_tengah">'.number_format(nsi_round($row['bunga_pinjaman'])).'</td>
						<td class="h_kanan">'.number_format(nsi_round($row['biaya_adm'])).'</td>
						<td class="h_tengah">'.number_format(nsi_round($row['jumlah_ags'])).'</td>
			...
			$html .= '<tr bgcolor="#eee">
						...
						<td class="h_tengah"><strong>'.number_format(nsi_round($jml_pokok)).'</strong></td>
						<td class="h_tengah"><strong>'.number_format(nsi_round($jml_bunga)).'</strong></td>
						<td class="h_kanan"><strong>'.number_format(nsi_round($jml_adm)).'</strong></td>
						<td class="h_tengah"><strong>'.number_format(nsi_round($jml_ags)).'</strong></td>
						<td></td>
					</tr>
				</table>';
		}
		...
		if(!empty($angsuran)) {
			...
			<tr class="header_kolom" >
				...
				<th style=" width:13%;"> Angsuran Ke </th>
				<th style=" width:17%;"> Bayar Pokok</th>
				<th style=" width:17%;"> Bayar Bunga</th>
				<th style=" width:18%;"> Jumlah Bayar</th>
			...
			foreach ($angsuran as $rows) {
				...
				$byr_tot 		+= $byr_pokok + $byr_bunga;

				$tot_pokok += $row['bayar_pokok'];
				$tot_bunga += $row['bayar_bunga'];
				$tot_bayar += $tot_pokok + $tot_bunga;
				...
				<td class="h_kanan"> '.number_format(nsi_round($rows->bayar_pokok)).'</td>
				<td class="h_kanan"> '.number_format(nsi_round($rows->bayar_bunga)).'</td>
				<td class="h_kanan"> '.number_format(nsi_round($rows->bayar_pokok + $rows->bayar_bunga)).'</td>
				</tr>';
			}
			
			$html.='
			<tr class="header_kolom">
				<td class="h_kiri" colspan="6"><strong>Jumlah</strong></td>
				<td class="h_kanan"><strong>'.number_format(nsi_round($dibayar)).'</strong></td>
...
/*<tr class="header_kolom">
				...
				<td class="h_kanan"><strong>'.number_format(nsi_round($rows->byr_pokok)).'</strong></td>
				<td class="h_kanan"><strong>'.number_format(nsi_round($byr_bunga)).'</strong></td>
				<td class="h_kanan"><strong>'.number_format(nsi_round($tot_bayar)).'</strong></td>
