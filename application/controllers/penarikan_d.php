...
class Penarikan_d extends OperatorController {
	
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('penarikan_d_m');
	...
	public function index() {
		...
		//panggil data
		$this->data['kas_id'] = $this->penarikan_d_m->get_data_kas();
		$this->data['jenis_id'] = $this->general_m->get_id_simpanan();

		$this->data['isi'] = $this->load->view('penarikan_list_d_v', $this->data, TRUE);
	...
	function get_jml_tabungan() {
		$id = isset($_POST['anggota_id']) ? $_POST['anggota_id'] : '';
		$jml= $this->penarikan_d_m->get_jml_simpanan($id);
	...
	function ajax_list() {
		...
		$data   = $this->penarikan_d_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
	...
	public function update($id=null) {
		...
		if($this->penarikan_d_m->update($id)) {
	...
	public function delete() {
		...
		if($this->penarikan_d_m->delete($id)) {
	...
	function cetak_laporan() {
		$penarikan = $this->penarikan_d_m->lap_data_penarikan();
		if($penarikan == FALSE) {
			//redirect('penarikan');
		...
		<table width="100%" cellspacing="0" cellpadding="3" border="1" border-collapse= "collapse">
			...
			foreach ($penarikan as $row) {
				$anggota= $this->penarikan_d_m->get_data_anggota($row->anggota_id);
				$jns_simpan= $this->penarikan_d_m->get_jenis_simpan($row->jenis_id);
