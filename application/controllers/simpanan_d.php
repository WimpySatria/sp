...
class Simpanan_d extends OperatorController {
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('simpanan_d_m');
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->simpanan_d_m->get_data_kas();
		$this->data['jenis_id'] = $this->general_m->get_id_simpanan(); 

		$this->data['isi'] = $this->load->view('simpanan_list_d_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$cari_simpanan = isset($_POST['cari_simpanan']) ? $_POST['cari_simpanan'] : '3101';
		...
		$data   = $this->simpanan_d_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['jumlah'] = number_format($r->jumlah_sebenarnya);
			$rows[$i]['lama'] = $r->lama.' Bulan';
	...
	function get_no_rek() {
		...
		$no_rek = $this->simpanan_d_m->get_no_rek($anggota_id, $jenis_id);
        echo json_encode($no_rek);
		exit();
	}
	
	function get_jumlah_simpanan() {
		$jenis_id = $this->input->post('jenis_id');
		$no_rek = $this->input->post('no_rek');
		$jumlah = $this->simpanan_d_m->get_jumlah($jenis_id, $no_rek);
	...
	// function get_jumlah_bunga() {
	// 	$this->simpanan_d_m->cek_bunga();
	// }
	...
	public function update($id=null) {
		...
		if($this->simpanan_d_m->update($id)) {
	...
	public function delete() {
		...
		if($this->simpanan_d_m->delete($id))
		{
	...
	function cetak_laporan() {
		...
		$simpanan = $this->simpanan_d_m->lap_data_simpanan($search,$sort,$order);
// 		print_r($simpanan);
// 		die();
		if($simpanan == FALSE) {
			//redirect('simpanan');
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Laporan Data Simpan DEPOSITO <br></span>
		...
		<tr class="header_kolom">
			...
			<th class="h_tengah" style="width:10%;"> No Trans</th>
	...
	public function get_norek_bunga()
	{
		$id_anggota = 340;
		echo json_encode($this->simpanan_d_m->get_norek_bunga($id_anggota));
	}




	public function bunga_deposito()
	{
		if($this->simpanan_d_m->cek_bunga()){
			echo "pembungaan";
		}else{
			echo " melebihi jatuh tempo";
		}
	}
}