...
class Cetak_simpanan extends OperatorController {
	...
	function cetak($id) {
		...
		// $config["per_page"] = 10;
		// $data_anggota = $this->lap_kas_anggota_m->get_data_anggota($config["per_page"], $offset); 
		// panggil seluruh data aanggota
		...
		$html .= ''.$pdf->nsi_box($text =' <table width="100%">
			...
			<tr>
				<td width="100%">BUKTI SETORAN TUNAI
					<hr width="100%">
				</td>
			</tr>
        ...
        //'.'AG'.sprintf('%04d', $row->anggota_id).'
			$html .='<table width="100%" text="bold">
			<tr>
			...
			<tr>
				<td width="50%" text-align="h_kiri" font-family="courier"> Alamat</td>

				<td>'.strtoupper($anggota->alamat).'</td>
			</tr>	
		...
		<p class="txt_content" width="100%">
			Informasi Hub : '.$out['telepon'].
		'<br/>
		<p>
		</p>
		<p>
		</p>
			<br align="right" txt_size="9pt">('.$row->user_name.')</br>
		</p>';
		ob_start();
		$pdf->nsi_html($html);
		ob_end_clean();
