...
class Angsuran extends OperatorController {
	...
	function ajax_list($id = NULL) {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['bayar_pokok'] = number_format(nsi_round($r->bayar_pokok));
			$rows[$i]['bayar_bunga'] = number_format(nsi_round($r->bayar_bunga));
	...
	function cek_sebelum_update() {
		...
		if($row_akhir->id_akhir != $id_bayar) {
			...
			// sudah dibayar
			$this->db->select('SUM(jumlah_bayar) AS jumlah_bayar, bayar_pokok, bayar_bunga');
			$this->db->where('pinjam_id', $master_id);
