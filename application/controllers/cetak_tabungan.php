...
class Cetak_tabungan extends OperatorController {
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('cetak_tabungan_m');
	...
	function cetak($id) {
		$tabungan = $this->cetak_tabungan_m->lap_data_tabungan($id);
		...
		$html .= ''.$pdf->nsi_box($text =' <table width="100%">
			...
			<tr>
				<td width="20%"><strong>BUKTI SETORAN</strong>
					<hr width="100%">
				</td>
		...
		foreach ($tabungan as $row) {
			$anggota= $this->cetak_tabungan_m->get_data_anggota($row->anggota_id);
			$jns_tabungan= $this->cetak_tabungan_m->get_jns_tabungan($row->jenis_id);
        ...
        //'.'AG'.sprintf('%04d', $row->anggota_id).'
			...
			<tr>
				<td> Jenis Akun </td>
				<td>:</td>
				<td>'.$jns_tabungan->jns_tabungan.'</td>
			</tr>
			<tr>
