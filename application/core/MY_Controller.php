...
class MY_Controller extends CI_Controller
{
	...
	public function __construct() {
		...
		if ($this->session->userdata('login') == FALSE) {
			...
			if($this->session->userdata('level') == 'operator' || $this->session->userdata('level') == 'cs'|| $this->session->userdata('level') == 'admin') {
...
class OPPController extends MY_Controller
{
	...
	public function __construct() {
		parent::__construct();
		// cek status level admin
		if ($this->session->userdata('level') == 'admin' || $this->session->userdata('level') == 'pinjaman' || $this->session->userdata('level') == 'operator' || $this->session->userdata('level') == 'cs') {
			//oke
			$this->data['akses'] = TRUE;
		} else {
			// no
