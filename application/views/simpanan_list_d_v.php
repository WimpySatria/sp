...
url="<?php echo site_url('simpanan_d/ajax_list'); ?>" 
...
<thead>
	<tr>
		...
		<th data-options="field:'id_txt', width:'10', halign:'center', align:'center'">Kode Trans</th>
		<th data-options="field:'no_rek', width:'25', halign:'center', align:'center'">No.Rek</th>
		...
		<th data-options="field:'anggota_id',halign:'center', align:'center'">ID</th>
		...
		<th data-options="field:'lama', width:'15', halign:'center', align:'right'">Lama Deposito</th>
...
<div id="tb" style="height: 35px;">
	<div style="vertical-align: middle; display: inline; padding-top: 15px;">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="update()">Ubah</a>
	...
	<div class="pull-right" style="vertical-align: middle;">
		...
		<input type="hidden" name="cari_simpanan" id="cari_simpanan" value="3101">
		...
		<!--<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" plain="false" onclick="cetakKoran()">Cetak Rekening Koran</a>-->
				...
				<td>
					<table>
						<tr style="height:35px">
							...
							<td>
								<div class="input-group date dtpicker col-md-5" style="z-index: 9999 !important;">
									...
									<input type="hidden" id="jenis_id" name="jenis_id" value="3101"/>
						...
						<tr style="height:35px">
							...
							<td>:</td>
							<td>
								<input class="easyui-numberbox easyui-validatebox" id="jumlah" name="jumlah" data-options="precision:0,groupSeparator:',',decimalSeparator:'.'" required="true" style="width:195px; height:25px"  />
							</td>	
						</tr>
						<tr style="height:35px">
							<td>Lama Deposito</td>
							<td>:</td>
							<td>
								<input id="lama" name="lama" type="number" value="1" min="1" /> Bulan
							</td>	
						</tr>
						<tr style="height:35px">
							...
							<td>:</td>
							<td>
								<select id="no_rek_bunga" name="no_rek_bunga" style="width:195px; height:25px" class="easyui-validatebox" required="true">
									<option value="0"> -- Pilih Nomor Rekening --</option>
								</select>
							</td>	
						</tr>
						<tr style="height:35px">
							...
							<td>:</td>
							<td>
								<textarea name="keterangan" id="keterangan" cols="30" rows="1"></textarea>
							</td>	
						</tr>
						<tr style="height:35px">
							<td>Simpan Ke Kas</td>
...
<script type="text/javascript">
	...
	function create(){
		...
		url = '<?php echo site_url('simpanan_d/create'); ?>';
	}
	...
	function update(){
		...
		if(row){
			...
			url = '<?php echo site_url('simpanan_d/update'); ?>/' + row.id;
			$('#jumlah ~ span input').keyup(function(){
				var val_jumlah = $(this).val();
				$('#jumlah').numberbox('setValue', number_format(val_jumlah));
			});
		...
		if (row){ 
			$.messager.confirm('Konfirmasi','Apakah Anda akan menghapus data kode transaksi : <code>' + row.id_txt + '</code> ?',function(r){  
				if (r){  
					$.ajax({
						type	: "POST",
						url		: "<?php echo site_url('simpanan_d/delete'); ?>",
						data	: 'id='+row.id,
						success	: function(result){
							var result = eval('('+result+')');
							$.messager.show({
	...
	function cetak () {
		var cari_simpanan 	= 3101;
		var kode_transaksi 	= $('#kode_transaksi').val();
		var tgl_dari			= $('input[name=daterangepicker_start]').val();
		var tgl_sampai			= $('input[name=daterangepicker_end]').val();
		
		var win = window.open('<?php echo site_url("simpanan_d/cetak_laporan/?cari_simpanan=' + cari_simpanan + '&kode_transaksi=' + kode_transaksi + '&tgl_dari=' + tgl_dari + '&tgl_sampai=' + tgl_sampai + '"); ?>');
		if (win) {
			win.focus();
		} else {
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#no_rek').on('change keyup', function(){
			const anggota_id = $('#anggota_id').val();
			
    		val_jenis_id = $('#jenis_id').val();
    		val_no_rek = $('#no_rek').val();
    		$.ajax({
    			url: '<?php echo site_url()?>simpanan_d/get_jumlah_simpanan',
    			type: 'POST',
    			dataType: 'html',
    		...
    		.done(function(result) {
				...
				$('#keterangan').val('Bunga deposito ('+val_no_rek+')');
    		})
    		.fail(function() {
		...
		$('#anggota_id').combogrid({
			panelWidth:400,
			url: '<?php echo site_url('simpanan_d/list_anggota'); ?>',
			idField:'id',
			valueField:'id',
			textField:'nama',
			mode:'remote',
			...
			onSelect: function(record){
				...
				$.ajax({
					url: '<?php echo site_url(); ?>simpanan_d/get_anggota_by_id/' + val_anggota_id,
					type: 'POST',
					dataType: 'html',
				...
				$.ajax({
					url : "<?=base_url('simpanan_d/get_norek_bunga')?>",
					data: {anggota_id : val_anggota_id},
					method : "POST",
					dataType : "JSON",
					success : function(hasil){
						$.each(hasil, function(i, rekening){
						$('#no_rek_bunga').append('<option value="'+rekening.no_rek+'">'+rekening.no_rek+'</option>');
						})
					}
		...
		function get_no_rek() {
    		...
    		console.log(val_jenis_id);
    		console.log(val_anggota_id);
    		$("#no_rek").empty();
    		...
    		$.ajax({
    			url: '<?php echo site_url()?>simpanan_d/get_no_rek',
    			type: 'POST',
    			dataType: 'html',
...
<script>
    ...
    function jumlah_bunga() {
		$.ajax({
			url: '<?php echo site_url()?>simpanan_d/get_jumlah_bunga',
			type: 'GET',
		})
		.done(function(data) {
			console.log(data);	
