...
<div class="box box-solid box-primary">
	<div class="box-header">
		<h3 class="box-title">Cetak Data Tabungan</h3>
...
<div class="box-body">
<p></p>
<p style="text-align:center; font-size: 15pt; font-weight: bold;"> Laporan Tabungan Anggota <br> Periode <?php echo $tgl_periode_txt; ?> </p>

<table  class="table table-bordered">
	<tr class="header_kolom">
		<th style="width:5%; vertical-align: middle; text-align:center" > No. </th>
		<th style="width:35%; vertical-align: middle; text-align:center">Jenis Akun </th>
		<th style="width:20%; vertical-align: middle; text-align:center"> Tabungan  </th>
	...
	foreach ($data_jns_Tabungan as $jenis) {
	if(($no % 2) == 0) {
		$warna="#FFFFCC"; } 
	else {
		$warna="#FFFFFF"; }

	$Tabungan_arr[$jenis->id] = $jenis->jns_simpan;
	$nilai_s = $this->lap_Tabungan_m->get_jml_Tabungan($jenis->id);
	$nilai_p = $this->lap_Tabungan_m->get_jml_penarikan($jenis->id);
...
function clearSearch(){
	window.location.href = '<?php echo site_url("lap_Tabungan"); ?>';
}
...
function doSearch() {
	...
	$('#fmCari').attr('action', '<?php echo site_url('lap_Tabungan'); ?>');
	$('#fmCari').submit();	
}
...
function cetak () {
	...
	//$('input[name=tgl_dari]').val(tgl_dari);
	//$('input[name=tgl_samp]').val(tgl_samp);
	//$('#fmCari').attr('action', '<?php echo site_url('lap_Tabungan/cetak'); ?>');
	//$('#fmCari').submit();
	var win = window.open('<?php echo site_url("lap_Tabungan/cetak/?tgl_dari=' + tgl_dari + '&tgl_samp=' + tgl_samp + '"); ?>');
	if (win) {
		win.focus();
	} else {
