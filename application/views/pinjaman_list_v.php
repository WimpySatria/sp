				...
				<td>
					<table>
						...
						<tr style="height:35px">
							...
							<td>:</td>
							<td>
							<input class="" id="lama_angsuran" name="lama_angsuran" style="width:195px; height:25px; background-color:#eee;"  />
							</td>	
						</tr>
						<tr style="height:35px">
							<td>Bunga</td>
							...
							<td>
								...
								<?php 
								$bunga = $this->db->get_where('suku_bunga',['opsi_key' => 'bg_pinjam'])->result();
								
								?>
								<select name="bunga" id="bunga" style="width:200px; height:23px" class="easyui-validatebox" required="true">
...
function create(){
	...
	jQuery('#bunga option[value="0"]').prop('selected', true);
	$("#anggota_poto").html('');
...
function save() {
	...
	if(bunga == 0) {
		$.messager.show({
			title:'<div><i class="fa fa-warning"></i> Peringatan ! </div>',
			msg: '<div class="text-red"><i class="fa fa-ban"></i> Maaf, Tentukan bunga terlebih dulu.</div>',
			timeout:2000,
			showType:'slide'
		});
		$("#bunga").focus();
